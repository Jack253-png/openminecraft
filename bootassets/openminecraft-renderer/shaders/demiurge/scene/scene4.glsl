...
#define RESOLUTION vec2(ubo.width, ubo.height)
#define TIME time.value
...
vec3 HSV2RGB(vec3 c)
{
    return c.z * mix(hsv2rgb_K.xxx,
                     clamp(abs(fract(c.xxx + hsv2rgb_K.xyz) * 6.0 - hsv2rgb_K.www) - hsv2rgb_K.xxx, 0.0, 1.0), c.y);
}
...
{
    vec3 sky = HSV2RGB(vec3(0.66, 0.2, 1.0));
    return sky;
}
